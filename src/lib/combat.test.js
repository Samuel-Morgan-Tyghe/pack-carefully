...
const processCombatTick = (
  ...
  const nextPlayerCooldowns = playerCooldowns.map((cd) => {
    ...
    if (current <= 0) {
      ...
      e.hp -= finalDmg
      if (finalDmg > 0)
        events.push(`Player's ${def.name} hits for ${Math.floor(finalDmg)}!`)

      if (def.combatStats?.heal)
        p.hp = Math.min(p.maxHp, p.hp + def.combatStats.heal)
...
const pCooldowns = [
  { instanceId: "1", itemId: "dagger", current: 10, max: 1000 },
]
