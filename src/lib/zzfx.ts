...
export const zzfx = (...parameters: (number | undefined)[]) => { 
    ...
    b: any[] = [], t=0, tm=0, i=0, j=1, r=0, c=0, s=0, f, length;
    ...
    for(length = attack + decay + sustain + release + delay | 0; i < length; b[i++] = s)
    {
        if(!(++c%(bitCrush*100|0)))                      // bit crush
        {
            ...
            s = (repeatTime ?
                ...
                1-((i-attack)/decay)*(1-sustainVolume) :  // decay_falloff
                i < attack  + decay + sustain ?           // sustain
                sustainVolume :                           // sustain_volume
                i < length - delay ?                      // release
                (length - i - delay)/release *            // release_falloff
                sustainVolume :                           // release_volume
                0);                                       // post_release

            s = delay ? s/2 + (delay > i ? 0 :            // delay
                (i<length-delay? 1 : (length-i)/delay) *  // release delay 
                b[i-delay|0]/2*1) : s;                    // else
        }
    ...
    let buffer = zzfxX.createBuffer(1, length, zzfxR), 
