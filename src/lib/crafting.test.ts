import { $itemsOnGrid, placeItem, resetGame, addPlayer, enterSandbox, SANDBOX_PLAYER_ID } from '../store/gameStore';
...
async function runCraftingTests() {
    ...
    // Test 1: Wooden Sword + Rock -> Hero Sword
    {
        ...
        console.log('Items on grid after placement:', items.map(i => i.itemId));
        ...
        if (hasHeroSword && !hasWoodSword && !hasRock) {
            console.log('✅ Crafting Successful: Hero Sword created, ingredients consumed.');
        } else {
            console.error('❌ Crafting Failed!');
            console.log('Status: HeroSword:', hasHeroSword, 'WoodSword:', hasWoodSword, 'Rock:', hasRock);
        }
    }
