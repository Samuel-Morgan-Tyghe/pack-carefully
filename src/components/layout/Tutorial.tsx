...
const tutorialSteps: TutorialStep[] = [
  {
    ...
    description: 'A social deduction game where you must pack your backpack strategically and survive the wilderness. But beware - one player is a traitor trying to sabotage the group!',
    icon: 'Map',
    tips: [
  ...
  },
  {
    ...
    description: 'Items MUST be placed inside containers (backpacks, pouches, pockets). You cannot place items in empty space!',
    icon: 'Backpack',
    tips: [
  ...
  },
  {
    ...
    description: 'Strategic placement matters! Items next to each other can provide bonuses or penalties.',
    icon: 'Sparkles',
    tips: [
...
const Tutorial: React.FC = () => {
  ...
  const Icon = (LucideIcons[currentTutorial.icon] || LucideIcons.HelpCircle) as React.ElementType;
                ...
                {currentTutorial.tips && (
                  <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700">
                    <div className="flex items-center gap-2 mb-4">
                      <LucideIcons.Lightbulb size={20} className="text-yellow-400" />
                    ...
                    <ul className="space-y-2">
                      {currentTutorial.tips.map((tip, idx) => (
                        <li key={idx} className="flex items-start gap-3 text-slate-300">
                          <LucideIcons.CheckCircle2 size={18} className="text-green-400 mt-0.5 flex-shrink-0" />
                          <span>{tip}</span>
                        </li>
                      ))}
                    </ul>
