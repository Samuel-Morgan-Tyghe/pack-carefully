...
const SupplyShelf: React.FC = () => {
    ...
    return (
        <section className="w-full md:w-80 h-auto md:h-full bg-wood-900/90 border-b-4 md:border-b-0 md:border-l-4 border-wood-700 shadow-2xl p-3 md:p-6 flex flex-col md:flex-col gap-3 md:gap-6 relative z-10 backdrop-blur-sm rounded-t-xl md:rounded-l-xl md:rounded-t-none">
            ...
            <div className="flex justify-between items-center border-b-2 border-wood-700 pb-2 shrink-0">
                <h3 className="font-display font-bold text-lg md:text-2xl text-gold-500 drop-shadow-sm flex items-center gap-2">
                    <LucideIcons.Package size={20} className="md:hidden" />
                    Supplies
                </h3>
            ...
            <div className="flex-1 overflow-x-auto md:overflow-y-auto pb-2 md:pr-2 flex flex-row md:flex-col gap-4 scrollbar-thin scrollbar-thumb-wood-600 scrollbar-track-wood-900 min-h-0">
                {/* Categories */}
                {['ESSENTIAL', 'TOOL', 'SURVIVAL', 'COMFORT', 'SABOTAGE'].map(cat => {
                    const items = availableItems.filter(i => i.category === cat);
                    if (items.length === 0) return null;
                    return (
                        <div key={cat} className="flex-shrink-0 md:flex-shrink-1">
                            <ShelfCategory
                                category={cat}
                                items={items}
                            />
