...
const DebugVerification: React.FC = () => {
  ...
  const log = (msg: string) =>
    setLogs((prev) => [
      ...prev,
      { id: Math.random().toString(36).substr(2, 9), text: msg },
    ])
  ...
  const runTests = async () => {
    ...
    try {
      ...
      // Check Morale
      if ($gameState.get().morale !== initialMorale - 15)
        throw new Error("Morale did not decrease correctly")
      ...
      const hasScrap = items.some(
        (i) => i.itemId === "rock" && i.ownerId === victimId,
      )

      if (!hasScrap)
        log(
          "⚠️ Loot placement failed (might be full? or random fail). Retrying forced placement...",
        )
      ...
      const itemBefore = $itemsOnGrid
        .get()
        .find((i) => i.ownerId === p2Id && i.itemId === "flashlight")
      if (!itemBefore) throw new Error("Failed to setup item for sabotage")

      log(
        `Item before: ${itemBefore.x}, ${itemBefore.y} (Rot: ${itemBefore.rotation})`,
      )
      ...
      const itemAfter = $itemsOnGrid
        .get()
        .find((i) => i.instanceId === itemBefore.instanceId)

      if (itemAfter) {
        if (
          itemAfter.x !== itemBefore.x ||
          itemAfter.y !== itemBefore.y ||
          itemAfter.rotation !== itemBefore.rotation
        ) {
          log(
            `✅ Item moved to: ${itemAfter.x}, ${itemAfter.y} (Rot: ${itemAfter.rotation})`,
          )
        } else {
  ...
  return (
    <div className="fixed bottom-4 left-4 z-50 bg-black/90 p-4 rounded border border-green-500 w-80 max-h-96 overflow-auto text-green-400 font-mono text-sm shadow-2xl">
      ...
      <div className="space-y-1">
        {logs.map((L) => (
          <div key={L.id} className="border-b border-green-900/30 pb-1">
            {L.text}
          </div>
        ))}
      </div>
