...
import { $gameState, $players, assignPlayerToPath, choosePath, returnToSplitScreen } from '../../store/gameStore';
...
const JourneySelection: React.FC = () => {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 text-white p-8">
            <h2 className="text-4xl font-black mb-2 uppercase tracking-widest text-yellow-500">Choose Your Path</h2>
            <p className="text-slate-400 mb-8 max-w-xl text-center">
                The group stands at a fork in the road. Split up to cover more ground, or stick together for safety?
            </p>
            ...
            <div className="flex gap-8 w-full max-w-5xl">
                ...
                <div className="flex-1 bg-red-900/20 border-2 border-red-900/50 rounded-xl p-6 relative hover:bg-red-900/30 transition-colors">
                    ...
                    <h3 className="text-2xl font-bold text-red-400 text-center mt-8 mb-4">The Dark Forest</h3>
                    
                    <div className="bg-black/30 rounded-lg p-4 min-h-[200px] mb-4">
                        {leftPathPlayers.map(p => (
                            <div key={p.id} onClick={() => assignPlayerToPath(p.id, null)} className="flex items-center gap-2 mb-2 cursor-pointer hover:opacity-75">
                    ...
                    <button 
                         onClick={() => undecidedPlayers.forEach(p => assignPlayerToPath(p.id, 'LEFT'))}
                ...
                <div className="w-48 flex flex-col items-center justify-center">
                     ...
                     <div className="bg-slate-800 p-4 rounded-lg w-full text-center">
                         <h4 className="text-xs uppercase font-bold text-slate-500 mb-2">Undecided</h4>
                         {undecidedPlayers.map(p => (
                            <div key={p.id} className="flex items-center justify-center gap-2 mb-2">
                ...
                <div className="flex-1 bg-indigo-900/20 border-2 border-indigo-900/50 rounded-xl p-6 relative hover:bg-indigo-900/30 transition-colors">
                    ...
                    <h3 className="text-2xl font-bold text-indigo-400 text-center mt-8 mb-4">The Ancient Ruins</h3>
                    ...
                    <button 
                         onClick={() => undecidedPlayers.forEach(p => assignPlayerToPath(p.id, 'RIGHT'))}
