...
const Finale: React.FC = () => {
  ...
  const isWin =
    ...
    (!gameState.gameResult && gameState.day >= 5) // Day 5 survival is win
  ...
  useEffect(() => {
    ...
    setScore(total)
  }, [gameState.day, gameState.morale])

  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-black/90 text-white relative overflow-hidden">
      {/* Background glow */}
      <div
        className={`absolute inset-0 opacity-20 ${isWin ? "bg-gradient-to-t from-gold-900 to-black" : "bg-red-900"}`}
      />
        ...
        {isWin ? (
          <>
            ...
            <p className="text-2xl text-slate-300">
              The expedition survived the wilderness.
            </p>
          </>
        ) : (
          <>
