...
import { $itemsOnGrid, moveItem, checkCollision, checkSupport, $players, $gameState, $containers } from '../../store/gameStore';
...
const MegaBag: React.FC = () => {
    ...
    const containers = useStore($containers); // NEW
    ...
    const [highlightedInstanceIds, setHighlightedInstanceIds] = useState<string[]>([]);
    ...
    const calculateGhostValidity = (gx: number, gy: number, itemId: string, instanceId?: string, currentRot: number = 0) => {
        ...
        const ownerId = item?.ownerId || players[0]?.id; // Fallback
        ...
        const noCollision = !checkCollision(gx, gy, w, h, items, ownerId, instanceId, itemDef.category);
    ...
    const totalPower = (combatInfo.stats.damage * 2) + combatInfo.stats.defense + Math.floor((100 + combatInfo.stats.block)/10);
        ...
        <div className="relative w-full h-full flex flex-col items-center justify-center">
            ...
            <div className="mt-8 text-center max-w-xl flex flex-col items-center gap-4">
                ...
                <p className="text-indigo-200 text-sm opacity-60">
                    <span className="font-bold text-red-400">CHAOS MODE:</span> All inventories are combined! 
                    Any player can move any item. Traitors, do your worst.
                </p>
            </div>
        </div>
    );
