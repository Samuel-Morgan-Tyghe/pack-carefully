...
const Journey: React.FC = () => {
  ...
  const events = [
    {
      title: "Desert Trek",
      description:
        "The sun is brutal. We need a Water Bottle to stay hydrated.",
  ...
  const hasItem = itemsOnGrid.some(
    (i) => i.itemId === currentEvent.requiredItem,
  )
        ...
        <div className="h-40 bg-forest-800 relative flex items-end p-6">
          ...
          <div className="absolute right-4 top-4 text-white/10">
            {React.createElement(
              ...
              )[currentEvent.icon] || LucideIcons.Map,
              { size: 120 },
            )}
          </div>
              ...
              <div className="p-3 bg-white/5 rounded-lg">
                {React.createElement(
                  (
                    ...
                    >
                  )[ITEMS[currentEvent.requiredItem]?.icon] ||
                    LucideIcons.HelpCircle,
                  { size: 32, className: "text-camp-orange" },
                )}
              </div>
              <div>
                ...
                <div className="text-xl font-bold text-white">
                  {ITEMS[currentEvent.requiredItem]?.name}
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="text-center animate-in fade-in slide-in-from-bottom-4">
              <div
                className={clsx(
                  ...
                  hasItem ? "text-green-400" : "text-red-500",
                )}
              >
